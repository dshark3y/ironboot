...
```bash
scp vps-bootstrap-v1.6.2.sh root@YOUR_SERVER_IP:~
```
...
```
Usage: sudo bash vps-bootstrap-v1.6.2.sh [options]
...
```bash
sudo bash vps-bootstrap-v1.6.2.sh --dry-run
```
...
```bash
sudo bash vps-bootstrap-v1.6.2.sh --verbose
```
...
```bash
sudo bash vps-bootstrap-v1.6.2.sh --only=docker,verify
```
...
```bash
sudo bash vps-bootstrap-v1.6.2.sh --skip=git,tailscale,close-ssh
```
...
```bash
sudo bash vps-bootstrap-v1.6.2.sh --only=tailscale
```

Setting up scheduled maintenance on an existing server:

```bash
sudo bash vps-bootstrap-v1.6.2.sh --only=cron
```
