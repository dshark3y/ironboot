...
```bash
scp vps-bootstrap-v1.6.3.sh root@YOUR_SERVER_IP:~
```

Or download it directly on the server:

```bash
curl -O https://raw.githubusercontent.com/dshark3y/ironboot/main/vps-bootstrap-v1.6.3.sh
```

**2. Run it as root**

```bash
sudo bash vps-bootstrap-v1.6.3.sh
```
...
```
Usage: sudo bash vps-bootstrap-v1.6.3.sh [options]
...
```bash
sudo bash vps-bootstrap-v1.6.3.sh --dry-run
```
...
```bash
sudo bash vps-bootstrap-v1.6.3.sh --verbose
```
...
```bash
sudo bash vps-bootstrap-v1.6.3.sh --only=docker,verify
```
...
```bash
sudo bash vps-bootstrap-v1.6.3.sh --skip=git,tailscale,close-ssh
```
...
```bash
sudo bash vps-bootstrap-v1.6.3.sh --only=tailscale
```

Setting up scheduled maintenance on an existing server:

```bash
sudo bash vps-bootstrap-v1.6.3.sh --only=cron
```
